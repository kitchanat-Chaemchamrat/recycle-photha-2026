    ...
    try {
        ...
        foreach ($sois as $soi_id) {
            $stmtScore = $pdo->prepare("
                ...
                WHERE h.soi_id = ? AND e.approval_status = 'approved' AND e.evaluation_date BETWEEN ? AND ?
            ");
            $stmtScore->execute([$soi_id, $week_start, $week_end]);
            $avg_score = $stmtScore->fetchColumn() ?: 0;

            $stmtWeight = $pdo->prepare("
                SELECT SUM(w.total_kg)
                FROM waste_weights w
                JOIN houses h ON w.house_id = h.id
                WHERE h.soi_id = ? AND w.record_date BETWEEN ? AND ?
            ");
        ...
        <div class="modal-content">
            <form method="POST">
                <div class="modal-header">
                    <h5 class="modal-title">คำนวณเงินปันผลรายสัปดาห์</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-6">
                            <label class="form-label">เริ่มสัปดาห์</label>
                            <input type="date" class="form-control" name="week_start" value="<?php echo $defaultStart; ?>" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label">สิ้นสุดสัปดาห์</label>
                            <input type="date" class="form-control" name="week_end" value="<?php echo $defaultEnd; ?>" required>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <div class="col-6">
                            <label class="form-label">หักกองทุนชุมชน (%)</label>
                            <input type="number" name="community_fund" class="form-control" value="10" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label">หักค่าแรงคัดแยก (%)</label>
                            <input type="number" name="worker_fund" class="form-control" value="10" required>
                        </div>
                    </div>
                    <div class="alert alert-info py-2 mb-0">ระบบจะดึงรายได้จาก `recycle_sales` ให้อัตโนมัติในช่วงสัปดาห์ที่เลือก</div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button>
                    <button type="submit" class="btn btn-primary-custom">คำนวณและบันทึก</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="periodDetailsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">รายละเอียดรอบสัปดาห์</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" id="periodDetailsBody">
